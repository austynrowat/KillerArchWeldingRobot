...
#AcqAddress()#might have bug with showing ascii
#AcqMValues()
AcqProfileSize()
...
def TransmitThread():
    while comPort:
        for n in range(0,100):
            if comPort:
                #inputstring=str(n%2)
                inputstring=(b'\x02\x00\x20\x1C\x03\x3C')
...
def LoopbackTest(comPortName):
    ...
    comPort=serial.Serial('/dev/ttyS0',9600,parity=serial.PARITY_NONE,stopbits=serial.STOPBITS_ONE,bytesize=serial.EIGHTBITS) # /dev/ttyACM0 for tx and rx, /dev/ttyUSB0 for usb ports, /dev/ttyS0 for uart
